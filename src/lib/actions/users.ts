...
export async function createUserAction(
  ...
  await writeAuditLog(session, "USER_CREATED", "USER", user.id, { role });
...
export async function updateUserRoleAction(userId: string, formData: FormData) {
  ...
  await writeAuditLog(session, "USER_ROLE_CHANGED", "USER", userId, { role });
...
export async function resetUserPasswordAction(userId: string, formData: FormData) {
  ...
  await writeAuditLog(session, "USER_PASSWORD_RESET", "USER", userId);
